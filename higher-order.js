...
//Code Here
let totalPopulation = populations.reduce((a, v) =>  {
  return a + v;
}, 0)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((v, i, a) =>  {
  return v.CP > 200;
})
...
let orderTotals = orders.map((v, i, a) => {
  return v.price * (v.tax + 1);
})
...
let bobsTotal = purchases.reduce((t, v, i, a) =>  {
  if (v.owner === 'Bob')
    return t + v.price;
  else
    return t;
