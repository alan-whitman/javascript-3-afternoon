...
const employeeUpdater = () => {
  employees.forEach((v, i, a) =>  {
    if (v.firstName === 'Theo')
      a.splice(i, 1);
    if (v.firstName === 'Lorie')
      a[i].department = 'HR'
  });
  return employees;
}
...
const removeDuplicates = () =>  {
  var exists = [];
  return workplaceAccidents.filter((v, i, a) => {
    if (exists.includes(v)) 
      return false;
    else {
      exists.push(v);
      return true;
...
const recordCleaner = () => {
  myCar.accidents.forEach((v, i) => {
    if (v.atFaultForAccident === true)
